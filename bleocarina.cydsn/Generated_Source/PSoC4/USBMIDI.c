...
void USBMIDI_Stop(void) 
{
    ...
    USBMIDI_CR0_REG &= (uint32) ~USBMIDI_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USBMIDI_USBIO_CR1_REG &= (uint32) ~ USBMIDI_USBIO_CR1_USBPUEN;
...
{
    uint32 cr1Reg = USBMIDI_CR1_REG;

    /* Clear bus activity. */
    USBMIDI_CR1_REG = (cr1Reg & (uint32) ~USBMIDI_CR1_BUS_ACTIVITY);
